    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {activeTab === "usuarios" && (
        <section className="section-shell p-6 md:p-8 space-y-5">
          ...
          <button className="rounded-full bg-primary text-primary-foreground px-5 py-2.5 text-sm font-semibold magnetic">Convidar novo usuário</button>
        </section>
      )}
              ...
              return (
                <article key={key} className={`rounded-2xl border p-4 transition-all ${
                  ...
                  {isCurrent ? (
                    ...
                    >
                      {Object.keys(PLAN_INFO).indexOf(key) > Object.keys(PLAN_INFO).indexOf(planoAtual) ? "Fazer upgrade" : "Alterar plano"}
                    </button>
                  )}
                </article>
              );
